...
class kwgPersonAndOrganization(models.Model):
	...
	the_person = models.ForeignKey(kwgPerson, on_delete=models.CASCADE, related_name="engaged_in", verbose_name="Personne")
	the_organization = models.ManyToManyField(kwgOrganization, related_name="engages", verbose_name="Organisation(s)")
	roles=models.ManyToManyField('utilisateur.kwgActorRole', blank=True)
	...
	def __str__(self):
		return self.the_person.user.first_name+" "+self.the_person.user.last_name+" agissant au nom de "+", ".join(self.the_organization.all().values_list('name', flat=True))
##################################
